...
module AiMethods
  ...
  SD_TOKENS = [
    ...
    "comfy", "parameters:", "seed:", "cfg:", "hires:",
  ].to_set.freeze

  # C2PA / Content Credentials indicators (usually in XMP/JUMBF)
  C2PA_TOKENS = %w[c2pa.org jumbf manifeststore c2pa].to_set.freeze
  ...
  def is_ai_generated?(file_path)
    return { score: 0, reason: "not an image" } unless is_file_image?(file_path)
    return { score: 0, reason: "file not found" } unless File.exist?(file_path)

    # Cache file type checks
    is_png = is_file_png?(file_path)
    is_jpeg = is_file_jpg?(file_path)
