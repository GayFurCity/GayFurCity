...
class PaginatorTest < ActiveSupport::TestCase
  ...
  { active_record: WikiPage, elasticsearch: Post }.each do |type, model| # rubocop:disable Metrics/BlockLength
    context(type) do
      setup do
        reset_post_index if type == :elasticsearch
        @user = create(:user)
      end
      ...
      should("fail for invalid page numbers") do
        ...
        assert_invalid_page_number(model, AdminConfig.instance.max_numbered_pages + 1)
        assert_invalid_page_number(model, "c1")
      end

      should("apply the correct limit") do
        assert_equal(AdminConfig.instance.records_per_page, model.paginate(1).records_per_page)
        assert_equal(10, model.paginate(1, limit: 10).records_per_page)
        assert_equal(10, model.paginate(1, limit: "10").records_per_page)
        assert_equal(AdminConfig.instance.max_per_page, model.paginate(1, limit: AdminConfig.instance.max_per_page + 1).records_per_page)
