...
module PostReplacementHelper
  def replacement_thumbnail(replacement)
    ...
    if replacement.metadata_only?
      image_tag(AdminConfig.instance.missing_preview_url)
    elsif replacement.post.deleteblocked?(CurrentUser.user)
      image_tag(AdminConfig.instance.deleted_preview_url)
