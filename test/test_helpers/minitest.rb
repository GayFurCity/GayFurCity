...
module Minitest
  module Assertions
    def assert_equal(exp, act, msg = nil)
      assert(exp == act, message(msg, nil) { diff(exp, act) }) # rubocop:disable Minitest/AssertOperator, Minitest/AssertEqual, Minitest/AssertWithExpectedArgument
    end
  end
end

# line number should be where the assert above is
Rails.backtrace_cleaner.add_silencer { |line| line =~ %r{test/test_helpers/minitest\.rb:9} }
